...
class FirebaseAnalytics {
  ...
  public readonly config: FirebaseAnalyticsConfig;
  private userId?: string;
  private userProperties?: { [key: string]: any };
  private eventQueue = new Set<FirebaseAnalyticsCodedEvent>();
  private options: FirebaseAnalyticsOptions;
   ...
   */
  private async send(events: Set<FirebaseAnalyticsCodedEvent>): Promise<void> {
  ...
  private async addEvent(event: FirebaseAnalyticsCodedEvent) {
  ...
  private async flushEvents() {
    ...
    const events = new Set<FirebaseAnalyticsCodedEvent>(this.eventQueue);
  ...
  static parseEvent(
    ...
    const params: FirebaseAnalyticsCodedEvent = {
      en: eventName,
      _et: Date.now(),
            ...
            (item: { [key: string]: string }, index: number) => {
              ...
              Object.keys(item).forEach((itemKey: string) => {
                if (SHORT_EVENT_ITEM_PARAMS[itemKey]) {
                  itemFields.push(
                    `${SHORT_EVENT_ITEM_PARAMS[itemKey]}${item[itemKey]}`
                  );
                } else {
  ...
  static parseUserProperty(
    ...
    if (
      !FirebaseAnalytics.isValidName(userPropertyName, 24) ||
  ...
  async logEvent(
    ...
    const event = FirebaseAnalytics.parseEvent(
      this.options,
      eventName,
      eventParams
    );
   ...
   */
  async setSessionTimeoutDuration(
...
function encodeQueryArgs(
  queryArgs: FirebaseAnalyticsCodedEvent,
  lastTime: number
): string {
...
const SHORT_EVENT_PARAMS: { [key: string]: string } = {
  currency: "cu",
};

// https://developers.google.com/gtagjs/reference/event
const SHORT_EVENT_ITEM_PARAMS: { [key: string]: string } = {
  id: "id",
  name: "nm",
  brand: "br",
  category: "ca",
